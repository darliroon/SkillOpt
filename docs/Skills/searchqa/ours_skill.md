...
## Output Compliance
- Follow any output syntax exactly, including answer tags such as `<answer>...</answer>`, and place only the concise answer inside them unless explanation is explicitly requested.

## Exact Surface Form
- For exact-match answers, select and copy the passage occurrence best matching the full clue rather than synthesizing a normalized form. Preserve its capitalization, spacing, number, articles, quotation marks, parentheses, hyphenation, apostrophes, diacritics, and punctuation. When equivalent forms occur, prefer the clue-aligned form at the requested granularity; do not expand it merely because another form is more formal.

- Before submitting, perform a slot-and-occurrence check: restate the prompt as a direct question, identify the words inside the answer slot, and copy only the contiguous clue-aligned occurrence filling it. Check articles, head nouns, modifiers, title components, number, capitalization, spelling, spacing, hyphenation, and punctuation; exclude words supplied outside the slot and avoid familiar or normalized substitutes.
...
- Before minimal-span extraction, determine whether the candidate is a conventional proper name, title, taxonomic label, or named geographic feature. Retain every clue-aligned component belonging to that name—including surnames, articles, plural endings, and inseparable type modifiers such as “Lake”—but add nothing absent from the clue-aligned occurrence.
- Copy the selected occurrence’s spelling and word boundaries literally; do not normalize joined or separated title forms, capitalization, number, or inflection.
